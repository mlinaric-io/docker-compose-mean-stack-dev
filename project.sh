...
if ! (( "$OSTYPE" == "gnu-linux" )); then
  echo "docker-compose-mean-stack-dev runs only on GNU/Linux operating system. Exiting..."
  exit
fi
...
{
    "name": "$PROJECT_NAME",
    "description": "docker-compose-mean-stack-dev project",
    "version": "1.0.0",
    "license": "MIT",
...
#JavaScript
  ...
  if [[ ! -d ${PROJECT_NAME} ]]; then
    docker-compose run node yarn add -D ynpx @angular/cli
    docker-compose run node yarn ng new $PROJECT_NAME
  fi

  docker-compose up -d
  sleep 5
  docker-compose -f docker-compose.yml up -d
  docker-compose run node sh -c "cd $PROJECT_NAME && yarn ng serve --open" -d
